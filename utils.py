...
def load_instance(filename):
    with open(filename, 'r') as sol:
        ...
        n_rectangle = int(sol.readline().strip())
        rectangles = []
        for i in range(n_rectangle):
            rectangles.append(tuple(int(el) for el in sol.readline().strip().split(" ")))
        return width, rectangles
...
def parse_arguments(main=True):
    ...
    if main:
        parser.add_argument("-S", "--solver", default="null", help="Select solver used for execution", choices=["null", "cp", "sat", "mip"])
